...
class TurboCommand(BaseCommand):
    """Command for interacting with the pollinations turbo service."""
    ...
    def description(self) -> str:
        return "Generate an image using the pollinations turbo service"
    ...
    @commands.command(name="turbo", aliases=["tdraw", "tgenerate", "t", "tu"])
    async def turbo(self, ctx: commands.Context, *, text: str):
        """Generate a stateless turbo response."""
        ...
        if not prompt:
            return await ctx.reply("❌ Please provide a prompt for turbo.")
        ...
        try:
            ...
            return await ctx.reply(f"❌ Turbo request failed: {exc}")
        
        session = getattr(self.bot, "session", None)
        
        image_file = discord.File(result, filename="turbo.png")
        ...
        async with self.ai_semaphore:
            async with session.post(self.nsfw_detetor_endpoint, data={"image": result}) as resp:
                if resp.status != 200:
                    ...
                    if is_nsfw:
                        result.seek(0)
                        image_file = discord.File(result, filename="SPOILER_turbo.png", spoiler=True)
        ...
        embed.set_footer(text=f"Seed: {used_seed}")
        embed.set_image(url="attachment://turbo.png")
        await ctx.message.remove_reaction(self.loading_emoji, self.bot.user)
        await ctx.reply(embed=embed, file=image_file)

    @discord.app_commands.command(name="turbo", description="Generate a response using the pollinations turbo service")
    @discord.app_commands.describe(prompt="The prompt to send to turbo", width="The width of the image", height="The height of the image")
    async def slash_turbo(
        self,
        ...
        try:
            ...
            return await interaction.followup.send(f"❌ Turbo request failed: {exc}", ephemeral=True)

        negative_prompt_val = None if negative_prompt == "worst quality, blurry" else negative_prompt
            ...
            async with session.post(self.nsfw_detetor_endpoint, data={"image": result}) as resp:
                if resp.status == 200:
                    ...
                    if data["is_nsfw"]:
                        ...
                        if host_channel:
                            msg = await host_channel.send(
                                content=f"NSFW Image Log (Slash)\nPrompt: {prompt}{f'\nNegative prompt: {negative_prompt_val}' if negative_prompt_val else ''}\nUser: {interaction.user} ({interaction.user.id})",
                                file=image_file,
                                delete_after=300
                            )
                            url = msg.attachments[0].url
...
async def setup(bot: commands.Bot):
    await bot.add_cog(TurboCommand(bot))
