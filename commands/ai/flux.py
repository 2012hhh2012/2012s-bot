...
class FluxCommand(BaseCommand):
    """Command for interacting with the pollinations flux service."""
    ...
    def description(self) -> str:
        return "Generate a image using the pollinations flux service"
    ...
    @commands.command(name="flux", aliases=["fdraw", "fgenerate", "f", "fl"])
    async def flux(self, ctx: commands.Context, *, text: str):
        """Generate a stateless flux response."""
        ...
        if not prompt:
            return await ctx.reply("❌ Please provide a prompt for flux.")
        ...
        try:
            ...
            return await ctx.reply(f"❌ Flux request failed: {exc}")
        
        session = getattr(self.bot, "session", None)
        
        image_file = discord.File(result, filename="flux.png")
        ...
        async with session.post(self.nsfw_detetor_endpoint, data={"image": result}) as resp:
            if resp.status != 200:
                ...
                if is_nsfw:
                    result.seek(0)
                    image_file = discord.File(result, filename="SPOILER_flux.png", spoiler=True)
    ...
    @discord.app_commands.command(name="flux", description="Generate a response using the pollinations flux service")
    @discord.app_commands.describe(prompt="The prompt to send to flux", width="The width of the image", height="The height of the image")
    async def slash_flux(
        self,
        ...
        try:
            ...
            return await interaction.followup.send(f"❌ Flux request failed: {exc}", ephemeral=True)
...
async def setup(bot: commands.Bot):
    await bot.add_cog(FluxCommand(bot))
