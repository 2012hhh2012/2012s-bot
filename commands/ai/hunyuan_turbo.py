...
class HunyuanTurboCommand(BaseCommand):
    """Command for interacting with the custom Hunyuan Turbo service."""
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.endpoint = f"{self.base_url}/2012/hunyuan-turbo" if self.base_url else None
    ...
    def description(self) -> str:
        return "Generate a response using the custom Hunyuan Turbo service"
        ...
        async with self.ai_semaphore:
            async with session.post(self.endpoint, json=payload, headers=headers) as response:
                if response.status != 200:
                    ...
                    raise RuntimeError(f"Hunyuan Turbo API returned {response.status}: {text}")

                data = await response.json()
                content = data.get("response")
                if not content:
                    raise RuntimeError("Hunyuan Turbo API response did not include a 'response' field.")
                return self._clean_response(content)

    def _clean_response(self, text: str) -> str:
        """Strip analysis blocks and labels from the output."""
        if not text:
            return text
    ...
    @commands.command(name="hunyuan-turbo", aliases=["hh", "huntrbo"])
    async def hunyuan_turbo(self, ctx: commands.Context, *, prompt: str):
        """Generate a stateless Hunyuan Turbo response."""
        ...
        if not prompt:
            return await ctx.reply("❌ Please provide a prompt for Hunyuan Turbo.")

        try:
            async with ctx.typing():
                result = await self._generate_response(prompt, user_id=str(ctx.author.id), name=ctx.author.name)
        except Exception as exc:  # pylint: disable=broad-except
            return await ctx.reply(f"❌ Hunyuan Turbo request failed: {exc}")

        messages = self.split_message(result)
        if not messages:
            return await ctx.reply("⚠️ Hunyuan Turbo returned an empty response.")

        await ctx.reply(messages[0])
        for message in messages[1:]:
            await ctx.send(message)

    @discord.app_commands.command(name="hunyuan-turbo", description="Generate a response using the custom Hunyuan Turbo service")
    @discord.app_commands.describe(prompt="The prompt to send to Hunyuan Turbo")
    async def slash_hunyuan_turbo(self, interaction: discord.Interaction, prompt: str):
        now = datetime.now(timezone.utc)
        ...
        try:
            ...
            return await interaction.followup.send(f"❌ Hunyuan Turbo request failed: {exc}", ephemeral=True)

        messages = self.split_message(result)
        if not messages:
            return await interaction.followup.send("⚠️ Hunyuan Turbo returned an empty response.")
...
async def setup(bot: commands.Bot):
    await bot.add_cog(HunyuanTurboCommand(bot))
