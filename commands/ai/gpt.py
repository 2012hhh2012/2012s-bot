...
class GPTCommand(BaseCommand):
    """Command for interacting with the custom GPT service."""
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.system_prompt = config.GPT_SYSTEM_INSTRUCTION
        self.endpoint = f"{self.base_url}/2012/gpt" if self.base_url else None
    ...
    def description(self) -> str:
        return "Generate a response using the custom GPT service"
        ...
        async with self.ai_semaphore:
            ...
            async with session.post(self.endpoint, json=payload, headers=headers) as response:
                if response.status != 200:
                    ...
                    raise RuntimeError(f"GPT API returned {response.status}: {text}")

                data = await response.json()
                content = data.get("response")
                if not content:
                    raise RuntimeError("GPT API response did not include a 'response' field.")
                return self._clean_response(content)

    def _clean_response(self, text: str) -> str:
        """Strip analysis blocks and labels from the GPT output."""
        if not text:
            return text
    ...
    @commands.command(name="gpt", aliases=["chatgpt", "chat-gpt"])
    async def gpt(self, ctx: commands.Context, *, prompt: str):
        """Generate a stateless GPT response."""
        ...
        if not prompt:
            return await ctx.reply("❌ Please provide a prompt for GPT.")

        try:
            async with ctx.typing():
                result = await self._generate_response(prompt, user_id=str(ctx.author.id), name=ctx.author.name)
        except Exception as exc:  # pylint: disable=broad-except
            return await ctx.reply(f"❌ GPT request failed: {exc}")

        messages = self.split_message(result)
        if not messages:
            return await ctx.reply("⚠️ GPT returned an empty response.")

        await ctx.reply(messages[0])
        for message in messages[1:]:
            await ctx.send(message)

    @discord.app_commands.command(name="gpt", description="Generate a response using the custom GPT service")
    @discord.app_commands.describe(prompt="The prompt to send to GPT")
    async def slash_gpt(self, interaction: discord.Interaction, prompt: str):
        ...
        try:
            ...
            return await interaction.followup.send(f"❌ GPT request failed: {exc}", ephemeral=True)

        messages = self.split_message(result)
        if not messages:
            return await interaction.followup.send("⚠️ GPT returned an empty response.")
...
async def setup(bot: commands.Bot):
    await bot.add_cog(GPTCommand(bot))
